...
function updateApiKeyStatus() {
  ...
  if (!span) return;
  if (getApiKey()) {
    span.textContent = '\u2713 API Key saved';
    span.style.color = '#15803d';
  } else {
    span.textContent = 'Set API Key (optional)';
    span.style.color = '';
  }
}

function openApiKeyModal() {
  el('apikey-input').value = getApiKey();
  el('apikey-modal').classList.add('open');
}
...
function saveApiKey() {
  ...
  updateApiKeyStatus();
  closeApiKeyModal();
  alert('API key saved! AI receipt scanning is now active.');
}
...
function renderDashboard() {
  ...
  el('dash-meetings-list').innerHTML = upcoming.length
    ? upcoming.map(m =>
        '<div class="meeting-item"><div class="meeting-dot"></div><div>' +
        '<div class="meeting-title">' + m.title + '</div>' +
        '<div class="meeting-time">' + fmtDate(m.date) + ' &bull; ' + m.time + '</div>' +
        '</div></div>').join('')
  ...
  el('dash-todos-preview').innerHTML = todos.slice(0, 3).map(t =>
    ...
    '<span style="' + (t.done ? 'text-decoration:line-through' : '') + '">' + t.text + '</span></div>'
  ).join('');
  ...
  el('dash-expenses-list').innerHTML = [...expenses].reverse().slice(0, 3).map(e =>
    ...
    '<span class="badge ' + (e.reimbursed ? 'reimbursed' : 'pending') + '">' + (e.reimbursed ? 'Reimbursed' : 'Pending') + '</span></div></div>'
  ).join('');
...
function toggleExp(id) {
  const e = expenses.find(x => x.id === id);
  if (e) e.reimbursed = !e.reimbursed;
...
function resetUploadArea() {
  ...
  el('upload-sub-text').textContent = 'PNG, JPG, WEBP or PDF \u2014 works without API key';
}
...
function saveExpense() {
  ...
  expenses.push({
    id: Date.now(), name, amount: Number(amount), purpose, date,
    reimbursed: false,
    receipt:    file ? file.name : '',
    receiptImg: currentReceiptDataUrl
  });
...
async function processReceipt(input) {
  ...
  if (!apiKey) {
    aiEl.textContent      = 'Receipt attached \u2014 fill in the fields below manually. Add an API key for AI auto-fill.';
    aiEl.style.background = '#f0f9ff';
    aiEl.style.color      = '#0369a1';
    aiEl.classList.add('visible');
    return;
  }

  aiEl.textContent      = '\ud83d\udd0d Scanning receipt...';
  ...
  try {
    ...
    if (!mt || mt === 'application/octet-stream') {
      ...
      mt = map[ext] || 'image/png';
    }

    const content = mt === 'application/pdf'
      ? [
          { type:'document', source:{ type:'base64', media_type:'application/pdf', data:b64 } },
          { type:'text', text:'Extract from this receipt PDF: merchant name, total amount (number only, no currency), and what was purchased. Reply ONLY as JSON: {"name":"...","amount":"...","purpose":"..."}' }
        ]
      : [
          { type:'image', source:{ type:'base64', media_type:mt, data:b64 } },
          { type:'text', text:'This is a receipt screenshot. Extract: merchant/store name, total amount paid (number only, no currency symbol), brief description of what was purchased. Reply ONLY as JSON with no markdown: {"name":"...","amount":"...","purpose":"..."}' }
        ];
    ...
    if (!resp.ok) {
      ...
      if (resp.status === 401) throw new Error('Invalid API key \u2014 check the key in the sidebar.');
    ...
    aiEl.textContent      = '\u2713 Fields auto-filled \u2014 please check before saving.';
    aiEl.style.background = '#ecfdf5';
    aiEl.style.color      = '#15803d';

  } catch (err) {
    console.error(err);
    aiEl.textContent      = '\u26a0\ufe0f ' + (err.message || 'Scan failed \u2014 fill in fields manually.');
...
function renderCalendar() {
  ...
  el('cal-headers').innerHTML = ['Mon','Tue','Wed','Thu','Fri','Sat','Sun']
  ...
  el('all-meetings-list').innerHTML = sorted.length
    ? sorted.map(m =>
        ...
        '<button class="meeting-del" onclick="deleteMeeting(' + m.id + ')">&#10005;</button></div>'
      ).join('')
...
function selectDate(ds) {
  ...
  el('day-meetings').innerHTML = dayMeets.length
    ? dayMeets.map(m =>
        '<div class="meeting-item"><div class="meeting-dot"></div>' +
        '<div><div class="meeting-title">' + m.title + '</div>' +
        '<div class="meeting-time">' + m.time + (m.notes ? ' &mdash; ' + m.notes : '') + '</div></div></div>'
      ).join('')
...
function openMeetingModal() {
  el('meeting-modal').classList.add('open');
  if (selectedDate) el('meet-date').value = selectedDate;
}
function closeMeetingModal() { el('meeting-modal').classList.remove('open'); }

function saveMeeting() {
  const title = el('meet-title').value.trim();
  const date  = el('meet-date').value;
  const time  = el('meet-time').value;
  const notes = el('meet-notes').value.trim();
...
function scheduleNotifications() {
  ...
  Notification.requestPermission().then(perm => {
    ...
    if (perm === 'granted') {
      banner.textContent = 'Notifications enabled \u2014 you will be reminded at 7:00 AM SGT daily.';
      scheduleDailyCheck();
    } else {
      banner.textContent = 'Notification permission denied. Enable notifications in your browser settings for daily 7 AM reminders.';
    }
  });
}
...
function saveSheet() {
  const name = el('sheet-name-input').value.trim();
  const url  = el('sheet-url-input').value.trim();
  if (!name || !url) { alert('Please fill in both fields.'); return; }
  sheets.push({ id: Date.now(), name, url });
  save(); closeSheetModal();
  el('sheet-name-input').value = '';
  el('sheet-url-input').value  = '';
  renderSheets(); renderDashboard();
}
function deleteSheet(id) {
  sheets = sheets.filter(s => s.id !== id);
  save(); renderSheets(); renderDashboard();
}
