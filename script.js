...
function updateApiKeyStatus() {
  ...
  if (!el) return;
  el.textContent = getApiKey() ? '✓ API Key Set' : 'Set API Key';
  el.style.color  = getApiKey() ? '#15803d'       : '#FF3B7F';
}
...
function clearExpForm() {
  ...
  ai.classList.remove('visible'); ai.textContent = '';
  ai.style.background = '';
  document.getElementById('upload-text').textContent = 'Upload Receipt (screenshot or photo)';
}
...
async function processReceipt(input) {
  ...
  if (!apiKey) {
    ai.textContent = '⚠️ No API key set. Click "Set API Key" in the sidebar to enable AI scanning. You can still fill in the fields manually.';
    ai.style.background = '#fffbeb';
    ai.classList.add('visible');
    return;
  }

  // Show file name on upload area
  document.getElementById('upload-text').textContent = '✓ ' + file.name;
  document.getElementById('upload-area').style.borderColor = '#FF3B7F';

  ai.textContent = '🔍 Reading your receipt...';
  ...
  try {
    ...
    // Determine media type — screenshots are usually PNG or JPEG
    ...
    // Build content array — images vs PDF handled differently
    let contentArr;
    if (mediaType === 'application/pdf') {
      contentArr = [
        {type:'document', source:{type:'base64', media_type:'application/pdf', data:b64}},
        {type:'text', text:'Extract from this receipt PDF: merchant/store name, total amount paid (number only, no currency symbol), and what was purchased. Respond ONLY as JSON with no extra text: {"name":"...","amount":"...","purpose":"..."}'}
      ];
    } else {
      contentArr = [
        {type:'image', source:{type:'base64', media_type:mediaType, data:b64}},
        {type:'text', text:'This is a receipt screenshot. Extract: merchant/store name, total amount paid (number only, no currency), and a brief description of what was purchased. Respond ONLY as JSON with no extra text or markdown: {"name":"...","amount":"...","purpose":"..."}'}
      ];
    }
    ...
    if (!resp.ok) {
      ...
      if (resp.status === 401) throw new Error('Invalid API key. Please check your key in "Set API Key".');
      if (resp.status === 429) throw new Error('Rate limit hit. Please wait a moment and try again.');
      throw new Error(msg);
    }
    ...
    ai.textContent = '✓ Receipt scanned! Fields auto-filled — please check and adjust if needed.';
    ai.style.background = '#ecfdf5';

  } catch(err) {
    console.error('Receipt scan error:', err);
    ai.textContent = '⚠️ ' + (err.message || 'Could not read receipt. Please fill in manually.');
    ai.style.background = '#fffbeb';
  }
}
...
function scheduleNotifications() {
  ...
  Notification.requestPermission().then(p => {
    ...
    if (p === 'granted') {
      ...
      banner.textContent = 'Notification permission denied. Enable notifications in your browser settings for daily 7 AM SGT reminders.';
    }
  });
}
